odoo.define('Original_webtreedynamic', function (require) {
    'use strict';
    ...
    ListRenderer.include({
        ...
        _renderBodyCell: function (record, node, colIndex, options) {
            ...
            var ctx = this.getEvalContext(record, node);
            // console.log(typeof(node.attrs.context));
            // console.log(node.attrs.context)
            // var py = window.py;
            // ctx = _.extend(ctx, py.eval(node.attrs.context));
        ...
        applyColorize: function ($td, record, node, ctx) {
            ...
            var nodeContext = node.attrs.context;
            if (_.isObject(nodeContext) && nodeContext != null) {
                // nodeContext should always be a string to be parsed
                nodeContext = JSON.stringify(nodeContext);
                ctx = _.extend(ctx, pyeval.py_eval(nodeContext));
            }
            // ctx = _.extend(ctx, pyeval.py_eval(nodeContext));
            // console.log("HI", typeof(node.attrs.context));
            // console.log(node.attrs.context)
            // console.log(nodeContext);
            console.log(typeof nodeOptions);

            if (!_.isObject(nodeOptions)) {
                console.log("NO, not an object");
                nodeOptions = pyeval.py_eval(nodeOptions);
                // console.log("eval", nodeOptions);
            }
